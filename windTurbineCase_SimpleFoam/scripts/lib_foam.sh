...
run_solver_serial() {
    local log="log.simpleFoam"

    echo ""
    echo "  simpleFoam  (serial)"
    ...
    setsid simpleFoam > "${log}" 2>&1 &
    local solver_pid=$!

    cleanup_serial() {
        echo ""
        echo "Interrompendo simpleFoam..."
    ...
    if [ "${status}" -eq 0 ]; then
        ...
        echo ""
        echo "  ERRO: simpleFoam encerrou com código ${status}." >&2
...
run_solver_parallel() {
    ...
    echo ""
    echo "  simpleFoam  (paralelo, ${np} processos)"
    echo "  log completo: ${log}"
    echo ""
    ...
    # Roda o solver
    setsid mpirun -np "${np}" simpleFoam -parallel > "${log}" 2>&1 &
    local mpi_pid=$!
    ...
    if [ "${status}" -eq 0 ]; then
        ...
        echo ""
        echo "  ERRO: simpleFoam paralelo encerrou com código ${status}." >&2
        echo "  Últimas linhas de ${log}:" >&2
        tail -30 "${log}" >&2
